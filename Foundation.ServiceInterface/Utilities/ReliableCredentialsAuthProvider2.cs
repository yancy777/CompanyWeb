...
{
    public class ReliableCredentialsAuthProvider2 : CredentialsAuthProvider
    {
...
//                    }
                    ...
                    if (reliableUserSession.RoleType == RoleType.Parent)
                    {
                        if (reliableUserSession.UserId != 0)
                        {
                            var childRelation =
                                db.Select(
                                    db.From<ParentChildRelation>().Where(c => c.UserId == reliableUserSession.UserId && c.IsSelected == true)).FirstOrDefault();
                            if (childRelation != null)
                            {
                                var childInfo =
                                    db.Select(db.From<DB_Child>().Where(c => c.Id == childRelation.ChildId))
                                   .FirstOrDefault();
//                                var child = db.Where<UV_Child>("UserId", reliableUserSession.UserId)
//                                    .OrderByDescending(c => c.IsSelected)
//                                    .FirstOrDefault();
                                if (childInfo != null)
                                {
                                    reliableUserSession.ChildId = childInfo.Id;
                                    reliableUserSession.ChildName = childInfo.ChildName;
                                    reliableUserSession.ClassId = childInfo.ClassId;
                                    reliableUserSession.KindergartenId = childInfo.ServiceUnitId;
...
//                                    reliableUserSession.MemberType = child.MemberType;
                                    ...
                                    reliableUserSession.Birthday = childInfo.Birthday.Value;
                                    reliableUserSession.Gender = childInfo.Gender;
                                    reliableUserSession.HeadImage = childInfo.ImageName;
                                }
                            }
                        }
                    }
                    else if (reliableUserSession.RoleType == RoleType.Teacher)
                    {
                        ...
                        {
                            if (reliableUserSession.ClassId >= 100000)
                            {
                                var classTable = db.Single<DB_Class>(c => c.Id == reliableUserSession.ClassId && c.Cancel == false);
                                if (classTable != null)
                                {
                                    reliableUserSession.ClassName = classTable.Name;
                                    reliableUserSession.ClassId = classTable.Id;
                                    var unitInfo =
                                        db.Single<DB_ServiceUnit>(
                                            u => u.Id == classTable.ServiceUnitId && u.Cancel == false);
                                    reliableUserSession.KindergartenName = unitInfo.Name;
                                }
                            }
                            else
                            {
